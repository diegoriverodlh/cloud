...
Este proyecto automatiza la generación de modelos de datos en formato **SQLX para Google Cloud Dataform** a partir de archivos de metadatos en formato **JSON** o **PDF**, o imágenes **(PNG, JPG, JPEG)** subidos a un bucket de **Google Cloud Storage (GCS)**.
...
### 4.1.4. main_v4.py (Código Servidor en la Nube y Dataform)

Este código está adaptado para procesar tanto invocaciones CloudEvent (en GCP 2ª Gen) como diccionarios estándar de Python (utilizados en pruebas locales). Añade el sqlx a Dataform. Permite trabajar con PDF, JPG o PNG entre otros y desde un diagrama crea todas las tablas correspondientes.
Clasifica el MIME type dinámicamente permitiendo procesar y enviar como bytes a Gemini formatos .png, .jpg y .pdf
```text
import json
import os
import re
...
def generate_sqlx_from_json(event, context=None):
    ...
    # Filtro de archivos soportados
    SUPPORTED_EXTENSIONS = (".json", ".pdf", ".png", ".jpg", ".jpeg")

    if (
        not file_name
        or file_name.startswith("output_sqlx/")
        or not file_name.lower().endswith(SUPPORTED_EXTENSIONS)
    ):
        print(f"Ignorando archivo no soportado o evento circular: {file_name}")
        return
    ...
    # ---------------------------------------------------------------------
    # RAMA 2: ARCHIVOS MULTIMODALES (PDF, PNG, JPG)
    # ---------------------------------------------------------------------
    elif file_name.lower().endswith((".pdf", ".png", ".jpg", ".jpeg")):
        print(f"Detectado archivo visual ({file_name}). Descargando bytes para Gemini...")
...
```text
INFO 2026-08-10T08:05:58.151608Z [httpRequest.requestMethod: POST] [httpRequest.status: 200] [httpRequest.responseSize: 130 B] [httpRequest.latency: 27.024 s] [httpRequest.userAgent: APIs-Google; (+https://developers.google.com/webmasters/APIs-Google.html)] https://function-2-se5qd5x3ca-ew.a.run.app/?__GCP_CloudEventsMode=GCS_NOTIFICATION
DEFAULT 2026-08-10T08:05:58.173599Z Procesando archivo: tablas.png del bucket: automatizacion-bucket-diego
DEFAULT 2026-08-10T08:05:58.183878Z Detectado archivo visual (tablas.png). Descargando bytes para Gemini...
DEFAULT 2026-08-10T08:06:24.205011Z Respuesta recibida de Gemini. Procesando archivos SQLX...
DEFAULT 2026-08-10T08:06:24.205314Z --- Procesando modelo final: afb-dimp-outstandbalancemonitoring-v0.sqlx ---
DEFAULT 2026-08-10T08:06:24.379058Z ✔ [GCS] Archivo SQLX guardado en: gs://automatizacion-bucket-diego/output_sqlx/main/afb-dimp-outstandbalancemonitoring-v0.sqlx
DEFAULT 2026-08-10T08:06:24.388063Z Subiendo archivo definitions/output_sqlx/afb-dimp-outstandbalancemonitoring-v0.sqlx a Dataform...
INFO 2026-08-10T08:06:24.467909Z [httpRequest.requestMethod: POST] [httpRequest.status: 200] [httpRequest.responseSize: 130 B] [httpRequest.latency: 2 ms] [httpRequest.userAgent: APIs-Google; (+https://developers.google.com/webmasters/APIs-Google.html)] https://function-2-se5qd5x3ca-ew.a.run.app/?__GCP_CloudEventsMode=GCS_NOTIFICATION
INFO 2026-08-10T08:06:24.476773Z Starting new instance. Reason: AUTOSCALING - Instance started due to configured scaling factors (e.g. CPU utilization, request throughput, etc.) or no existing capacity for current traffic.
DEFAULT 2026-08-10T08:06:24.573521Z ✔ Archivo 'afb-dimp-outstandbalancemonitoring-v0.sqlx' guardado correctamente.
DEFAULT 2026-08-10T08:06:24.573535Z Compilando proyecto de Dataform...
DEFAULT 2026-08-10T08:06:25.079760Z ✔ Compilación completada: projects/gen-lang-client-0449946907/locations/europe-west1/repositories/mi-repositorio-dataform/compilationResults/f8260a78-c240-4b5c-9236-d9c2da65f55d
DEFAULT 2026-08-10T08:06:25.079771Z Iniciando ejecución del modelo en BigQuery...
DEFAULT 2026-08-10T08:06:25.191382Z ✔ Target detectado en compilación: 'afb-dimp-outstandbalancemonitoring-v0' (Schema: dataform)
DEFAULT 2026-08-10T08:06:25.198198Z Ignorando archivo no soportado o evento circular: output_sqlx/main/afb-dimp-outstandbalancemonitoring-v0.sqlx
INFO 2026-08-10T08:06:27.411874Z Default STARTUP TCP probe succeeded after 1 attempt for container "worker" on port 8080.
```
---
